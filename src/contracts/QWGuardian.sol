...
abstract contract QWGuardian is AccessControl {
    ...
    bytes32 public constant GUARDIAN_OPERATOR_ROLE = keccak256("GUARDIAN_OPERATOR_ROLE");
    ...
    modifier onlyGuardian() {
        if (!_guardians[msg.sender]) revert QWGuardian__onlyGuardian_notGuardian();
        _;
    }
    ...
    function initializeGuardianOperator(address operator) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "QWGuardian: Must have admin role to initialize");
        _grantRole(GUARDIAN_OPERATOR_ROLE, operator);
    }
}
