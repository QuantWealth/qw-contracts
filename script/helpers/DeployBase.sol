...
contract DeployBase is Script {
  ...
  error InvalidRegistryAddress(); // Error for invalid registry address
  ...
  function _readBaseEnvVariables() internal view returns (DeploymentBaseParams memory baseParams) {
    ...
    baseParams.qwRegistry = vm.envAddress('QW_REGISTRY');
    if (baseParams.qwManager == address(0)) revert InvalidRegistryAddress();
  }
}
